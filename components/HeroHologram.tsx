...
const NODES: NodeDef[] = [
  { label: "肩こり", sub: "肩・首", x: 0.4, y: 0.8, z: 0.18, side: "r" },
  { label: "腰痛", sub: "腰まわり", x: 0.3, y: 0.18, z: 0.22, side: "r" },
  { label: "膝痛", sub: "ひざ・歩行", x: -0.22, y: -0.86, z: 0.18, side: "l" },
];
...
export function HeroHologram() {
  ...
  useEffect(() => {
    ...
    function build() {
      ...
      // ── 人体（y上, 原点=骨盤付近） ──
      addEllipsoid(0, 1.34, 0.03, 0.23, 0.29, 0.25, C(300)); // 頭
      addVTube(1.06, 0.86, 0.13, 0.11, 0.15, 0.12, C(70)); // 首
      addVTube(0.88, 0.3, 0.46, 0.24, 0.3, 0.19, C(540)); // 胸→腰
      addVTube(0.3, -0.02, 0.3, 0.19, 0.42, 0.24, C(360)); // 腰→骨盤
      addEllipsoid(-0.44, 0.82, 0.02, 0.18, 0.16, 0.17, C(90)); // 左肩
      addEllipsoid(0.44, 0.82, 0.02, 0.18, 0.16, 0.17, C(90)); // 右肩
      addTube(-0.46, 0.8, 0.03, -0.6, 0.3, 0.08, 0.15, 0.12, C(150)); // 左上腕
      addTube(-0.6, 0.3, 0.08, -0.7, -0.14, 0.14, 0.12, 0.095, C(130)); // 左前腕
      addEllipsoid(-0.72, -0.22, 0.15, 0.1, 0.14, 0.08, C(60)); // 左手
      addTube(0.46, 0.8, 0.03, 0.6, 0.3, 0.08, 0.15, 0.12, C(150));
      addTube(0.6, 0.3, 0.08, 0.7, -0.14, 0.14, 0.12, 0.095, C(130));
      addEllipsoid(0.72, -0.22, 0.15, 0.1, 0.14, 0.08, C(60));
      addTube(-0.18, -0.02, 0.0, -0.22, -0.86, 0.03, 0.22, 0.15, C(240)); // 左太もも
      addTube(-0.22, -0.86, 0.03, -0.22, -1.58, 0.0, 0.15, 0.095, C(190)); // 左すね
      addEllipsoid(-0.22, -1.66, 0.16, 0.11, 0.07, 0.2, C(60)); // 左足
      addTube(0.18, -0.02, 0.0, 0.22, -0.86, 0.03, 0.22, 0.15, C(240));
      addTube(0.22, -0.86, 0.03, 0.22, -1.58, 0.0, 0.15, 0.095, C(190));
      addEllipsoid(0.22, -1.66, 0.16, 0.11, 0.07, 0.2, C(60));
      ...
      for (let i = 0; i < n; i++) {
        ...
        brA[i] = Math.random() < 0.08 ? 0.011 + Math.random() * 0.009 : 0.0048 + Math.random() * 0.004;
        // 集合/散らばり用の乱数ベクトル（球状）
        const u = Math.random() * 2 - 1;
        const th = Math.random() * Math.PI * 2;
        const s = Math.sqrt(1 - u * u);
